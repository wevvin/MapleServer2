...
namespace MapleServer2.Packets
{
    public static class TriggerPacket
    {
        ...
        public static Packet Banner(BannerState state, int stringGuideId)
        {
            PacketWriter pWriter = PacketWriter.Of(SendOp.TRIGGER);
            pWriter.WriteEnum(TriggerPacketMode.Banner);
            pWriter.WriteEnum(state); // 02 = on, 03 = off
        ...
        public static Packet Timer(int time, bool startCountdown) //needs revisiting
        {
            PacketWriter pWriter = PacketWriter.Of(SendOp.TRIGGER);
            ...
            pWriter.WriteBool(startCountdown); //not true..this is broken
            pWriter.WriteInt();
            pWriter.WriteShort();
            return pWriter;
        }
